...
New-Item -ItemType Directory -Path "$tmp\admin", "$tmp\ui", "$tmp\i18n", "$tmp\deploy" | Out-Null
...
Push-Location $tmp
tar -czf $tar admin ui i18n deploy
Pop-Location
...
cp -a /tmp/jetbay-admin-unpack/i18n/. "$APP_ROOT/vendor/i18n/"
mkdir -p "$APP_ROOT/deploy"
cp -a /tmp/jetbay-admin-unpack/deploy/. "$APP_ROOT/deploy/"
chmod +x "$APP_ROOT/deploy"/*.sh 2>/dev/null || true
python3 -c 'import json; from pathlib import Path; p=Path("/var/www/jetbay-admin/package.json"); d=json.loads(p.read_text()); d["name"]="jetbay-admin"; d.setdefault("dependencies",{})["@jetbay/ui"]="file:vendor/ui"; d.setdefault("dependencies",{})["@jetbay/i18n"]="file:vendor/i18n"; p.write_text(json.dumps(d,indent=2)+"\n"); print("patched",p)'
echo "[sync-admin] OK"
'@
