...
**Source of truth:** code audit nhánh `jetvina` · cập nhật **2026-07-18**  
...
| -------------------- | ----------------------------------------------------------------------------- | -------- |
| Login staff          | `login/page.tsx` reject mọi role ≠ `ADMIN`                                    | P0       |
| Auth gate            | `AdminAuthGate` chỉ check token localStorage                                  | P0       |
| 403 handling         | `api.ts` coi `403` như session expired → clear token / logout                 | P0       |
...
- Users API DTO + UI only allow `USER` \| `ADMIN`; cannot promote/manage `SALES` / `CONTRACT_APPROVER` safely.
