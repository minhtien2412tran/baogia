...
| Account dashboard/profile/trips | `/account/*` `/account/bookings` | users | `/account/dashboard`, `/me`, `/me/avatar` | GET/PATCH/POST | JWT | `AccountContext` + profile editor + trip dashboard | DONE | skeleton | retry + 401→login | avatar/contact/social fields, trip management link, staff system-dashboard link | auth-booking | PROD · UAT visual pending | T-S3-02 |
