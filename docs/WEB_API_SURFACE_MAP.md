...
| Account dashboard/profile/trips | `/account/*` `/account/bookings` | users | `/account/dashboard`, `/me`, `/me/avatar` | GET/PATCH/POST | JWT | `AccountContext` + sectioned profile editor + trip dashboard | DONE | skeleton | retry + 401→login + network error | stable `User.publicId`; internal numeric ID stays BE-only in profile surfaces; avatar/contact/social fields sync through shared DB | auth-booking | migration `20260721005500_user_public_id` · PROD UAT pending | T-S3-02 |
