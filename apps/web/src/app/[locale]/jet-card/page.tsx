    ...
    <>
      ...
      >
        <section className="jb-sub-section">
          ...
          <FeatureGrid items={JB.pages.jetCard.benefits.map((b) => ({ icon: b.icon, title: b.title, body: 'Book anytime, anywhere — just a tap away.' }))} />
        </section>
            ...
            {data.plans.map((p: Record<string, unknown>) => {
              ...
              return (
                <div key={String(p.id)} className="jb-jetcard-item">
                  ...
                  <div style={{ fontWeight: 600 }}>{copy.subtitle}</div>
                  <div className="jb-jetcard-subtitle">{copy.desc}</div>
                  <a href="#jet-card-enquiry" className="jb-unlock-link">Unlock Now ›</a>
                </div>
              );
            })}
          </div>
