...
describe('PermissionService resolution order', () => {
  ...
  it('ADMIN always allowed', async () => {
    await expect(service.hasPermission(1, 'ADMIN', 'booking.cancel')).resolves.toBe(true);
  ...
  it('DENY overrides ROLE allow', async () => {
    ...
    await expect(service.hasPermission(2, 'SALES', 'booking.cancel')).resolves.toBe(false);
  ...
  it('ALLOW overrides missing role permission', async () => {
    ...
    await expect(service.hasPermission(2, 'SALES', 'contract.approve')).resolves.toBe(true);
  });

  it('INHERIT falls through to role permission', async () => {
    prisma.userPermissionOverride.findUnique.mockResolvedValue({ effect: 'INHERIT' });
    prisma.rolePermission.findUnique.mockResolvedValue({ permission: 'booking.view' });
    await expect(service.hasPermission(2, 'SALES', 'booking.view')).resolves.toBe(true);
  });
  ...
  it('no override uses role only', async () => {
    ...
    await expect(service.hasPermission(2, 'SALES', 'permission.manage')).resolves.toBe(false);
  });
});
